...
void Ball::Update(float deltaseconds)
{
	if(!m_HasLaunched)
	{
		...
		if(g_InputSystem->GetMousePosition().m_X >= m_Dims.m_X / 2 && g_InputSystem->GetMousePosition().m_X <= MAGNUS_WINDOW_DIMS[1] - m_Dims.m_X / 2)
	    {
	    	m_Position.m_X = g_InputSystem->GetMousePosition().m_X;
	    }
	    else if(g_InputSystem->GetMousePosition().m_X > MAGNUS_WINDOW_DIMS[1] - m_Dims.m_X / 2)
	    {
	    	m_Position.m_X = MAGNUS_WINDOW_DIMS[1];
	    }
	    else if(g_InputSystem->GetMousePosition().m_X < m_Dims.m_X / 2)
	    {
	    	m_Position.m_X = MAGNUS_WINDOW_DIMS[0];
	    }
	}

	if(m_HasLaunched)
	{
		if(m_Position.m_X - m_Radius < MAGNUS_WINDOW_DIMS[0] || m_Position.m_X + m_Radius > MAGNUS_WINDOW_DIMS[1])
	    {
	    	m_Velocity.m_X *= -1;
	    }
	    
	    if(m_Position.m_Y - m_Radius < MAGNUS_WINDOW_DIMS[0] || m_Position.m_Y + m_Radius > MAGNUS_WINDOW_DIMS[3])
	    {
	    	m_Velocity.m_Y *= -1;
	    }
	}
	...
	{
		if(m_Position.m_X - 100.0f < MAGNUS_WINDOW_DIMS[0])
	    {
	    	m_Position.m_X = 100.0f;
	    }
		else if(m_Position.m_X + 100.0f > MAGNUS_WINDOW_DIMS[1])
		{
...
void Ball::DebugRender()
{
	...
	m_ModelData.m_Model = Mat4::Translation(Vec3(m_Position.m_X, m_Position.m_Y, 0.0f));
