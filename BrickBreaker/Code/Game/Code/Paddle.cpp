...
void Paddle::Update(float deltaseconds)
{
	...
	{
		m_Position.m_X = MAGNUS_WINDOW_DIMS[1] - m_Dims.m_X;
	}
...
void Paddle::DebugRender()
{
	...
	if(m_Owner->m_IsPaddleBallCollision)
	{
		...
		m_ModelData.m_Model = Mat4::Translation(Vec3(refPoint.m_X, refPoint.m_Y, 0.0f));
		g_Renderer->DrawArrow(start, end, 3.0f, Color::YELLOW, m_ModelData);
